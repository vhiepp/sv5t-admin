...
const TABLE_HEAD = [
  ...
  { id: "stu_code", label: "Mssv", alignRight: false },
  ...
  if (query) {
    return filter(array, (_user) => {
      return (
        ...
        `${_user.stu_code}`.toLowerCase().indexOf(query.toLowerCase()) !== -1 ||
...
export default function UserPage() {
  ...
  useEffect(() => {
    axiosApi.post("/admin/user").then(({ data }) => {
      setUserList(data);
    });
  }, []);
  ...
  return (
    <>
      <Helmet>
        <title> User | Sinh viên 5 tốt TVU </title>
      </Helmet>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, fullname, role, stu_code, avatar, email } = row;
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell align="left">{stu_code}</TableCell>

                        <TableCell align="left">{row.class?.toUpperCase()}</TableCell>
