...
import { Box, Link, Card, Grid, Avatar, Typography, CardContent, IconButton, Modal, Button, Snackbar } from '@mui/material';
...
export default function BlogPostCard({ post, index, url, ...props }) {
  ...
  const handleDeletePost = () => {
    ...
    axiosApi.post(url.delete, { slug: post.slug })
      .then(({ data }) => {
  ...
  const handleActivePost = () => {
    ...
    axiosApi.post(url.active, { slug: post.slug })
      .then(({ data }) => {
...

      {
        ...
        >
          <Box sx={{ ...style, width: 490, borderRadius: 2 }}>
            ...
            <p>
              <small>
                <i>*Mẹo: Sau khi xóa, {props.typeName} sẽ được đưa đến mục <b>Đã xóa (Deleted)</b>, bạn có thể vào mục này để khôi phục lại.</i>
              </small>
            </p>
            <hr />
            <Box sx={{
              ...
              <Button variant="outlined" sx={{ marginRight: 2 }} size="medium" color="error" onClick={handleDeletePost} disabled={loadingModalDelete}>
                Xóa
              </Button>
...

      {
        ...
        >
          <Box sx={{ ...style, width: 490, borderRadius: 2 }}>
            ...
            <p>
              <small>
                <i>*Mẹo: Sau khi duyệt, {props.typeName} sẽ được đưa đến mục <b>Hiển thị (Show)</b>, bạn có thể vào mục này để xóa hoặc chỉnh sửa {props.typeName}.</i>
              </small>
            </p>
            <hr />
            <Box sx={{
              ...
              <Button variant="outlined" sx={{ marginRight: 2 }} size="medium" color="success" onClick={handleActivePost} disabled={loadingModalActive}>
                Duyệt
              </Button>
